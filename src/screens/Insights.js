...
export default Insight = props => {
  ...
  const insights = useSelector(state => state.common.insights);
  ...
  const fetchCategories = () => {
    ...
    dispatch(commonActions.getInsightCats(params));
  };
  ...
  const goToInsightCategory = category => {
    props.navigation.push('InsightsCategories', {category: category});
  };
  ...
  const renderHeader = () => {
    return (
      <LinearGradient colors={['#ffffff', '#cee6ff']} style={{height:responsiveHeight(18),marginBottom:responsiveHeight(1.5)}} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}>
      <View style={{...styles.featureTitle, marginTop:responsiveHeight(3), marginBottom:responsiveHeight(2),justifyContent:"space-between"}}>
        <Text style={{...styles.psTitle,position:"relative",top:responsiveHeight(0.3),backgroundColor:"transparent"}}>Categories</Text>
      </View>
      {insightCats.length > 0 && 
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} contentContainerStyle={{ flexDirection:"row",alignItems:"center",paddingHorizontal:responsiveWidth(3),height:responsiveHeight(6),}}>
          <View style={stylesInline.jobWrapper}>
            { insightCats.map((category, index) => {
              return (
                <TouchableOpacity key={index} onPress={() => goToInsightCategory(category)}>
                  <View style={stylesInline.categoryContent}>
                    <Text style={stylesInline.categoryTitle} numberOfLines={2} ellipsizeMode='tail'>{category.name}</Text>
                  </View>
                </TouchableOpacity>
              );
            })}
        ...
        <TouchableWithoutFeedback onPress={() => goToInsightDetails(insight)}>
          <View style={index % 2 == 0 ? stylesInline.workshopContentLeft : stylesInline.workshopContentRight}>
            ...
            <View style={stylesInline.workshopContentContainer}>
              ...
              {insight.en.short_description ?
              <Text style={stylesInline.workshopDesc} numberOfLines={2} ellipsizeMode='tail'>
                {insight.en.short_description}
              </Text>
              :
              <Text style={stylesInline.workshopDesc} numberOfLines={2} ellipsizeMode='tail'>
                {htmlToText(insight.en.description, {wordwrap: false})}
              </Text>
              }
  ...
  if (isLoading) {
    return (
      <LinearGradient colors={['#ffffff', '#e3f0ff']} style={{...stylesInline.screen,flex: 1,justifyContent: 'center',alignItems: 'center',}} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}>
  ...
  return (
    ...
    <LinearGradient colors={['#ffffff', '#e3f0ff']} style={{flex: 1,}} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}>
      ...
      <View>
        <FlatList 
          style={{flex: 1,paddingBottom:responsiveHeight(5),minHeight:responsiveHeight(91)}}
